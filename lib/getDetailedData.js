...
const fetchPricesWithRetry = async (retries, delay) => {
  for (let i = 0; i < retries; i++) {
    try {
      const priceRequestAvalanche = await fetch(redstoneApiAvalanche);
      if (!priceRequestAvalanche.ok) {
        throw new Error(
          `Failed to fetch prices for Avalanche. Status: ${priceRequestAvalanche.status}`
        );
      }
      const pricesAvalanche = await priceRequestAvalanche.json();

      // console.log('pricesAvalanche', pricesAvalanche);

      const priceRequestArbitrum = await fetch(redstoneApiArbitrum);
      if (!priceRequestArbitrum.ok) {
        throw new Error(
          `Failed to fetch prices for Arbitrum. Status: ${priceRequestArbitrum.status}`
        );
      }
      const pricesArbitrum = await priceRequestArbitrum.json();

      // console.log('pricesArbitrum', pricesArbitrum);

      const prices = {...pricesArbitrum, ...pricesAvalanche};
...
const getAvalancheBoostRates = async (symbols, prices, provider) => {
  ...
  await Promise.all(
    symbols.map(async (symbol) => {
      ...
      try {
        console.log(
          `[${new Date().toISOString()}] Fetching boost rate for ${symbol}`
        );
        ...
        console.log(
          `[${new Date().toISOString()}] Fetched boost rate for ${symbol}`
        );
      } catch (error) {
...
export const fetchCryptoData = async () => {
  ...
  await Promise.all(
    Object.keys(pools).map(async (network) => {
      console.log(
        `[${new Date().toISOString()}] Fetching data for network: ${network}`
      );
      ...
      await Promise.all(
        Object.entries(pools[network]).map(async ([asset, address]) => {
          ...
          if (code === "0x") {
            console.error(
              `Contract code not found at address: ${address} on network: ${network}`
            );
            throw new Error(
              `No contract code at address ${address} for ${asset}`
            );
          }
        })
      );
      ...
      const poolsInfo = await Promise.all(
        Object.entries(pools[network]).map(async ([asset, address]) => {
          ...
          const fetchWithRetry = async (retries, delay) => {
              ...
              try {
                ...
                console.log(
                  `[${new Date().toISOString()}] Attempt ${
                    i + 1
                  } failed for ${asset}. Retrying in ${delay}ms...`,
                  error
                );
                await new Promise((resolve) => setTimeout(resolve, delay));
                delay *= 2;
            ...
            throw new Error(
              `Failed to fetch details for ${asset} after several attempts`
            );
          };
