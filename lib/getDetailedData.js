  ...
  for (let network of Object.keys(pools)) {
    ...
    allPoolsData[network] = Object.keys(pools[network])
      .map((symbol) => {
        ...
        console.log(`Raw values for ${symbol}:`, { apy, tvl, price });

        if (!tvl) {
          console.warn(`TVL is undefined for symbol: ${symbol}`);
        }
        ...
        if (tvl) {
          ...
          console.log(`TVL in USD for ${symbol}:`, tvlInUsd);
          ...
          console.warn(`TVL is zero or undefined for symbol: ${symbol}`);
        }
