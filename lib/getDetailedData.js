...
import { decimals } from "./constants"; // Importing the predefined decimals
...
const getAvalancheBoostRates = async (symbols, prices, provider) => {
  ...
  await Promise.all(
    symbols.map(async (symbol) => {
      ...
      try {
        console.log(`[${new Date().toISOString()}] Fetching boost rate for ${symbol}`);
        ...
        console.log(`[${new Date().toISOString()}] Fetched boost rate for ${symbol}`);
      } catch (error) {
...
export const fetchCryptoData = async () => {
  ...
  await Promise.all(
    Object.keys(pools).map(async (network) => {
      console.log(`[${new Date().toISOString()}] Fetching data for network: ${network}`);
      ...
      await Promise.all(
        Object.entries(pools[network]).map(async ([asset, address]) => {
          ...
          if (code === "0x") {
            console.error(`Contract code not found at address: ${address} on network: ${network}`);
            throw new Error(`No contract code at address ${address} for ${asset}`);
          }
        })
      );
      ...
      const poolsInfo = await Promise.all(
        Object.entries(pools[network]).map(async ([asset, address]) => {
          console.log(`[${new Date().toISOString()}] Fetching data for asset: ${asset} with address: ${address}`);
          const poolContract = new ethers.Contract(address, POOL.abi, provider);

          const decimalsForAsset = decimals[asset.toUpperCase()] || 18;
          console.log(`Using predefined decimals for ${asset}: ${decimalsForAsset}`);
          ...
          const fetchWithRetry = async (retries, delay) => {
            for (let i = 0; i < retries; i++) {
              try {
                ...
                const totalSupply = ethers.utils.formatUnits(poolDetails[0], decimalsForAsset);
                const depositRate = ethers.utils.formatUnits(poolDetails[1], 18);
                const totalBorrowed = ethers.utils.formatUnits(poolDetails[2], decimalsForAsset);
                ...
                console.log(
                  `[${new Date().toISOString()}] Attempt ${i + 1} failed for ${asset}. Retrying in ${delay}ms...`,
                  error
                );
      ...
      const fetchPricesWithRetry = async (retries, delay) => {
        for (let i = 0; i < retries; i++) {
          try {
            console.log(
              `[${new Date().toISOString()}] Fetching prices from Redstone API (Attempt ${i + 1})`
            );
            ...
            console.log(`[${new Date().toISOString()}] Prices fetched`);
            return prices;
          } catch (error) {
            console.log(`[${new Date().toISOString()}] Attempt ${i + 1} failed. Retrying in ${delay}ms...`, error);
            await new Promise((resolve) => setTimeout(resolve, delay));
            delay *= 2;
          }
        }
      ...
      const [prices, LTIPPoolsBoost] = await Promise.all([
        fetchPricesWithRetry(5, 1000),
        fetch(LTIPPoolBoostApi).then((response) => response.json()),
      ]);

      console.log(`[${new Date().toISOString()}] LTIP Pools Boost data fetched`);

      const AvalancheBoostRates =
        network === "avalanche"
          ? await getAvalancheBoostRates(Object.keys(pools["avalanche"]), prices, provider)
          : {};
      ...
      allPoolsData[network] = Object.keys(pools[network])
        .map((symbol) => {
          ...
          console.log(`[${new Date().toISOString()}] Processing pool data for symbol: ${symbol}`);
          ...
          if (network === "arbitrum") {
            miningApr = LTIPPoolsBoost[symbol] ? LTIPPoolsBoost[symbol] * 100 : 0;
          } else {
            miningApr = AvalancheBoostRates[symbol]
              ? (AvalancheBoostRates[symbol] / (parseFloat(poolInfo.totalSupply) * price)) * 100
              : 0;
          }
          ...
          // Formatting total supply for display
          let formattedTotalSupply = "";
          if (totalSupplyInUsd < 1_000_000) {
            formattedTotalSupply = `$${(totalSupplyInUsd / 1_000).toFixed(2)}k`;
          } else {
            formattedTotalSupply = `$${(totalSupplyInUsd / 1_000_000).toFixed(2)}M`;
          }

          console.log(`Formatted Total Supply for ${symbol}: ${formattedTotalSupply}`);

          return {
            symbol,
            apy: isNaN(combinedApy) ? apy : combinedApy, // Use APY if combined APY is NaN
            miningApr: isNaN(miningApr) ? 0 : miningApr, // Set mining APR to 0 if NaN
            availableLiquidity,
            totalBorrowedInUsd,
            price,
            formattedTotalSupply, // Adding formatted total supply
          };
        })
  ...
  console.log(`[${new Date().toISOString()}] Final allPoolsData:`, allPoolsData);
  console.log(`[${new Date().toISOString()}] Total Available Liquidity (USD):`, totalAvailableLiquidity.toFixed(2));
  console.log(`[${new Date().toISOString()}] Total Borrowed Liquidity (USD):`, totalBorrowedLiquidity.toFixed(2));
  return {
