...
// const BlogPreviewCards = ({ blogProp }) => {
//   const blogData = blogProp.map((blog) => {
...
//         <p>LATEST BLOsssssssssssG title WAS {latestBlog.fields.blogTitle}</p>
...
export const BlogPreviewCards = ({ blogProp }) => {
  console.log(blogProp);

  return (
    <div>
      {" "}
      {blogProp.map((blog, index) => (
        <div className="my-2 border-blue-400 border-2" key={index}>
          <h2>{blog.blogTitle}</h2>
          <p>{blog.blogDescription}</p>
          <p>Minutes to read: {blog.minsToRead}</p>
